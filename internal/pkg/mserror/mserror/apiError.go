...
func handle(c *gin.Context) {
	err := doSomething()
	if err != nil {
		// extrai code do APIError
		if code, ok := APIError.CodeOf(err); ok {
...
func Wrap(code int, msg string, cause error) *APIError {
	return &APIError{Code: code, Message: msg, Cause: cause}
}

func Wrapf(code int, cause error, format string, args ...any) *APIError {
